...
interface IModule {
    ...
    function getModuleInfo() external view returns (address, string memory, string memory, string memory, address[] memory, uint256, uint256, uint256, address);
...
contract Module is OwnableUpgradeable, IModule , ReEntrancyGuard {
    ...
    function getModuleInfo() external override view returns (address, string memory, string memory, string memory, address[] memory, uint256, uint256, uint256, address){
        address[] memory _holders = new address[](holders.length());
        for(uint256 i = 0; i < holders.length(); i++){
            _holders[i] = holders.at(i);
        }

        return (token, IERC20(token).name(), IERC20(token).symbol(), moduleDetails, _holders, IERC20(token).totalSupply(), totalStaked, emission, owner());
    }
